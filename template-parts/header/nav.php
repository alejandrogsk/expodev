...
//For MENU
$menu_class = \EXPODEV_THEME\Inc\Menus::get_instance();
$header_menu_id = $menu_class->get_menu_id('expodev-header-menu');
        ...
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          foreach ($header_menus as $menu_item ) {
            if (!$menu_item->menu_item_parent ) {
              //$child_menu_items are all the child menus for one item. ej: parent id 1 = child 1 and child 2
              $child_menu_items = $menu_class->get_child_menu_items($header_menus, $menu_item->ID);
              $has_childern = ! empty($child_menu_items) && is_array($child_menu_items);
              $has_sum_menu_items = ! empty( $has_childern ) ? 'has-submenu' : '';
