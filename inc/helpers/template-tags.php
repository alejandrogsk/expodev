...
function expodev_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'expodev' ),
		'<a href="' . esc_url( get_permalink() ) . '>' . $time_string . '</a>'
	);

	echo '<span class="text-secondary">' . $posted_on . '</span>';
}
 ...
 * @return void
 */
function expodev_posted_by() {
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'expodev' ),
		'<span class=""><a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);

	echo '<span class="text-secondary">' . $byline . '</span>';
}
...
function expodev_the_excerpt( $trim_character_count = 0 ) {
 ...
 */
function expodev_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="text-decoration-none" href="%1$s"><button class="mt-2 mb-4 btn btn-info text-white ">%2$s</button></a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'expodev' )
		);
	}
...
function expodev_pagination() {
	...
	printf( '<nav class="clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}